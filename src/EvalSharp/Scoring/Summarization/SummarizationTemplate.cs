namespace EvalSharp.Scoring.PromptSummarization;

internal static class SummarizationTemplate
{
    //Prompt adapted from deepeval, licensed under Apache 2.0
    //DeepEval Repo: https://github.com/confident-ai/deepeval
    public static string GenerateReason(IEnumerable<string>? contradictions, IEnumerable<string>? redundancies, IEnumerable<string>? questions, double score)
    {
        string formattedContradictions = contradictions.ToFormattedList();

        string formattedRedundancies = redundancies.ToFormattedList();

        string formattedQuestions = (questions != null && questions.Any())
            ? $"""
Questions the original text can answer but not the summary:
{questions.ToFormattedList()}
...
{{formattedRedundancies}}

{{formattedQuestions}}JSON:
""";
    }

    //Prompt adapted from deepeval, licensed under Apache 2.0
    //DeepEval Repo: https://github.com/confident-ai/deepeval
    public static string GenerateAnswers(IEnumerable<string> questions, string text)
    {
        string formattedQuestions = questions.ToFormattedList();
...
""";
    ...
    //Prompt adapted from deepeval, licensed under Apache 2.0
    //DeepEval Repo: https://github.com/confident-ai/deepeval
    public static string GenerateQuestions(string text, int n)
    {
        return $$"""
...
""";
    ...
    //Prompt adapted from deepeval, licensed under Apache 2.0
    //DeepEval Repo: https://github.com/confident-ai/deepeval
    public static string GenerateAlignmentVerdicts(IEnumerable<string> summaryClaims, string originalText)
    {
        string joinedClaims = summaryClaims.ToFormattedList();
