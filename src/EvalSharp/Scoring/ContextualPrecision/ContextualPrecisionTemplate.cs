...
namespace EvalSharp.Scoring.ContextualPrecision
{
    internal static class ContextualPrecisionTemplate
    {
        public static string GenerateVerdicts(string input, string expectedOutput, List<string> retrievalContext)
        {
            //Prompt adapted from deepeval, licensed under Apache 2.0
            //DeepEval Repo: https://github.com/confident-ai/deepeval

            // Create a string representing the document count
            string docCountStr = $" ({retrievalContext.Count} document{(retrievalContext.Count > 1 ? "s" : "")})";
            string retrievalContextJson = JsonSerializer.Serialize(retrievalContext);
            return $$"""
        ...
        public static string GenerateReason(string input, string verdicts, string score)
        {
            //Prompt adapted from deepeval, licensed under Apache 2.0
            //DeepEval Repo: https://github.com/confident-ai/deepeval
            return $$"""
